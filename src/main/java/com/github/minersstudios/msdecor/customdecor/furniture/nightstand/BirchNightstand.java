...
public class BirchNightstand implements Wrenchable {
	...
	public BirchNightstand() {
		this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), "birch_nightstand");
	...
	public enum Type implements Typed.Type {
		...
		Type(int customModelData) {
			this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), this.name().toLowerCase(Locale.ROOT) + "_birch_nightstand");
