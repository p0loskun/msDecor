...
public class JungleNightstand implements Wrenchable {
	...
	public JungleNightstand() {
		this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), "jungle_nightstand");
	...
	public enum Type implements Typed.Type {
		...
		Type(int customModelData) {
			this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), this.name().toLowerCase(Locale.ROOT) + "_jungle_nightstand");
			this.itemName = "Тропическая тумбочка";
