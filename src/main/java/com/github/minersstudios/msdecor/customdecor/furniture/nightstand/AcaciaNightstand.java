...
public class AcaciaNightstand implements Wrenchable {
	...
	public AcaciaNightstand() {
		this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), "acacia_nightstand");
	...
	public enum Type implements Typed.Type {
		...
		Type(int customModelData) {
			this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), this.name().toLowerCase(Locale.ROOT) + "_acacia_nightstand");
			this.itemName = "Акациевая тумбочка";
