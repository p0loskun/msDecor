...
public class MangroveNightstand implements Wrenchable {
	...
	public MangroveNightstand() {
		this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), "mangrove_nightstand");
	...
	public enum Type implements Typed.Type {
		...
		Type(int customModelData) {
			this.namespacedKey = new NamespacedKey(MSDecor.getInstance(), this.name().toLowerCase(Locale.ROOT) + "_mangrove_nightstand");
